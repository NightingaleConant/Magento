...
class Mage_Sales_Model_Order extends Mage_Sales_Model_Abstract
{
    ...
    {
        if ($this->getPayment()->getOrder()->getInstallmentTypeId()) {
...
/*********************** STATUSES ***************************/
    ...
    protected function _checkState()
    {
        ...
        $hasForced = $this->hasForcedCanCreditmemo();
        $totalRefund = $this->getTotalRefunded();
        ...
        if ($this->getInstallmentTypeId()) {

            if ($this->getStatus() == 'ready_to_print' && !$this->canShip()) {
                $this->setState(self::STATE_PROCESSING, 'shipped', '', $userNotification);
            }

            if ($this->getTotalRefunded() && $this->getState() !== self::STATE_CLOSED) {
                $installment = Mage::getModel('installment/master')->getCollection();
                $installment->addFieldToFilter('order_id', $this->getIncrementId())
                ...
                foreach($installment as $transaction) {
                    $totalPayment += $transaction->getInstallmentMasterAmountDue();
                }

                if ($totalPayment == $totalRefund) {
                    $this->_setState(self::STATE_CLOSED, true, '', $userNotification);
                }
            }
        }
